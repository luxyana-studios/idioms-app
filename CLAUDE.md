...
npx supabase status         # check if local stack is running + show URLs/keys
npx supabase db reset       # drop local DB, reapply migrations + seed
...
- **Local development**: runs a full Postgres + Auth + REST stack in Docker via `npx supabase start`
- **Config**: `supabase/config.toml` — ports, auth settings, seed file paths
- **Migrations**: `supabase/migrations/` — timestamped SQL files, applied in order by `db reset`
- **Seed data**: `supabase/seed.sql` — runs after migrations on `db reset`
- **Client**: `src/core/supabase/client.ts` — configured with `detectSessionInUrl: false` (React Native)
...
The app requires a running local Supabase stack for data features. First-time setup:
1. Install Docker and ensure it's running
2. `npx supabase start` (first run pulls images, takes a few minutes)
3. Copy the **Project URL** and **Publishable** key from the output into `.env`
4. `npx supabase db reset` to apply migrations and seed data

Without `.env` configured, the app falls back to placeholder mode and logs a console warning.
Auth and database features won't work until the local stack is running and `.env` is set.
...
1. `npx supabase migration new <table_name>` — creates a timestamped file in `supabase/migrations/`
2. Write the SQL: `create table`, `enable row level security`, `create policy`
3. Add seed data to `supabase/seed.sql` if needed
4. `npx supabase db reset` — applies migration + seed to local DB
5. `npx supabase gen types typescript --local > src/types/supabase.ts` — regenerate types
6. Run `npx biome check --write src/types/supabase.ts` — the generator doesn't emit semicolons
7. Create a query hook or store fetch method in the relevant feature
8. Use `snake_case` for Postgres columns, map to `camelCase` in the app layer
