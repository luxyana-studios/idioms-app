...
| Topic | File |
|---|---|
| Product mission & USP | [docs/product/mission.md](docs/product/mission.md) |
| Features & screens | [docs/product/features.md](docs/product/features.md) |
...
- **AI-mined content:** LangChain pipeline seeds idioms into staging. Human review required before promoting to prod.
- **Auth:** Supabase SSO — Google + Apple (Apple mandatory for iOS App Store) + email fallback
- **Paywall:** RevenueCat, introduced when the app reaches ~10k users
- **Design system:** Aetheric Terra — terracotta + cream palette, glassmorphism

## Rules

- **No Claude attribution in commits.** Never add `Co-Authored-By` lines, `Generated by Claude`, or any other Claude/AI attribution to commit messages.
- **Fix root causes, not symptoms.** Never add one-off workarounds (error-swallowing, defensive checks for things that shouldn't happen, fallback shims). Identify the underlying reason a problem occurs and fix it there.
